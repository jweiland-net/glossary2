...
*   [TASK] Require TYPO3 14 LTS; drop support for TYPO3 13 and below
*   [TASK] Replace removed ``CharsetConverter::specCharsToASCII()`` with ``utf8_char_mapping()``
*   [TASK] Inject ``CharsetConverter`` via DI instead of instantiating it directly
*   [TASK] Remove ``TypoScriptFrontendController`` usage removed from page title provider test
*   [TASK] Rename default Fluid template to ``Glossary.fluid.html`` to match TYPO3 14 naming convention
*   [TASK] Remove TCA columns auto-generated by ``TcaEnrichment`` (``sys_language_uid``, ``l10n_parent``, ``l10n_source``, ``hidden``)
*   [TASK] Add ``translationSource`` to ``ctrl`` section of glossary TCA
*   [TASK] Remove redundant ``format: datetime`` from ``starttime`` and ``endtime`` TCA fields
*   [TASK] Move ``allowLanguageSynchronization`` for ``images`` field from ``appearance.behaviour`` to ``behaviour``
*   [TASK] Replace deprecated ``ExtensionManagementUtility::addPiFlexFormValue()`` with direct ``columnsOverrides`` assignment
*   [TASK] Remove obsolete ``internal_type: db`` from FlexForm page selector fields
*   [TASK] Remove obsolete ``if (!defined('TYPO3'))`` guards from TCA override files
*   [REFACTOR] Apply Rector fixes: privatize final class properties and methods, use ``readonly`` where applicable
*   [REFACTOR] Convert anonymous ``is_numeric`` closure to first-class callable syntax
